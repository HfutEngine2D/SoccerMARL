...
class LowActionSoccerEnv(HighActionSoccerEnv):
    """
    LowActionSoccerEnv is the same task as SoccerEmptyGoal, which tasks the 
    ...
    def __init__(self, config):
        super(LowActionSoccerEnv, self).__init__(config)
        # dash, turn, kick, tackle
        low0 = np.array(
            [0, -180], dtype=np.float32
